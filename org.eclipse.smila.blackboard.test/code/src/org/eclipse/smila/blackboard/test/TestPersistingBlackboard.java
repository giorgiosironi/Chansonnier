...
public class TestPersistingBlackboard extends DeclarativeServiceTestCase {
  ...
  public void testSynchronize() throws Exception {
    ...
    _blackboard.commit(id);
    _blackboard.invalidate(id);
    final Record newRecord = RecordFactory.DEFAULT_INSTANCE.createRecord();
    newRecord.setId(id);
    final Literal newValue = newRecord.getFactory().createLiteral();
    newValue.setStringValue("SMILA");
    final Attribute newAttribute = newRecord.getFactory().createAttribute();
    newAttribute.setLiteral(newValue);
    newRecord.getMetadata().setAttribute("topic", newAttribute);
    _blackboard.synchronize(newRecord);
    Path path = new Path("author[1]");
    assertEquals(_blackboard.getLiteral(id, path).getValue().toString(), "l2");
    path = new Path("author[1]/firstName1");
    assertEquals(_blackboard.getLiteral(id, path).getValue().toString(), "Georg");
    assertNull(_blackboard.getLiteral(id, new Path("unexistingPath")));
    path = new Path("topic");
    assertEquals(_blackboard.getLiteral(id, path).getValue().toString(), "SMILA");
  }
  ...
  public void testSynchronizeWithoutOldRecord() throws Exception {
    final Id id = IdFactory.DEFAULT_INSTANCE.createId("another-source", "another-key");
    _blackboard.invalidate(id);
    final Record newRecord = RecordFactory.DEFAULT_INSTANCE.createRecord();
    newRecord.setId(id);
    final Literal newValue = newRecord.getFactory().createLiteral();
    ...
    newRecord.getMetadata().setAttribute("topic", newAttribute);
    _blackboard.synchronize(newRecord);
    Path path = new Path("topic");
    assertEquals(_blackboard.getLiteral(id, path).getValue().toString(), "SMILA");
    path = new Path("author[1]/firstName1");
    assertNull(_blackboard.getLiteral(id, path));
    assertNull(_blackboard.getLiteral(id, new Path("unexistingPath")));
  }
