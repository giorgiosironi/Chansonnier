 ...
 * 
 * This File is based on the plugin/parse-html/src/java/org/apache/nutch/parse/html/DOMBuilder.java from Nutch 0.8.1 
...
public class DOMBuilder implements ContentHandler, LexicalHandler {
   ...
   * The Log.
   */
  private static final Log LOG = LogFactory.getLog(DOMBuilder.class);
   ...
   *           the SAX exception
   */
  protected void append(final Node newNode) throws org.xml.sax.SAXException {
   ...
   *           the SAX exception
   */
  public void endElement(final String ns, final String localName, final String name)
    throws org.xml.sax.SAXException {
    m_elemStack.pop();
   ...
   *           the SAX exception
   */
  public void charactersRaw(final char ch[], final int start, final int length) throws org.xml.sax.SAXException {
    if (isOutsideDocElem() && XMLCharacterRecognizer.isWhiteSpace(ch, start, length)) {
      return; // avoid DOM006 Hierarchy request error
    }
   ...
   *           the SAX exception
   */
  public void entityReference(final String name) throws org.xml.sax.SAXException {
   ...
   *           the SAX exception
   */
  public void comment(final char ch[], final int start, final int length) throws org.xml.sax.SAXException {
   ...
   * @see org.xml.sax.Locator
   */
  public void cdata(final char ch[], final int start, final int length) throws org.xml.sax.SAXException {
    if (isOutsideDocElem() && XMLCharacterRecognizer.isWhiteSpace(ch, start, length)) {
      return; // avoid DOM006 Hierarchy request error
    }
    ...
    final Node n = m_currentNode.getLastChild();
  ...
  public void startPrefixMapping(final String prefix, final String uri) throws org.xml.sax.SAXException {
     ...
     * 0)) || (null == prefix)) qname = "xmlns"; else qname = "xmlns:"+prefix;
     * 
     * Element elem = (Element)m_currentNode; String val = elem.getAttribute(qname); // Obsolete, should be DOM2...?
