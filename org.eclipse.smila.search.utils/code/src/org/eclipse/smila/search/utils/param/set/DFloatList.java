...
public class DFloatList extends DParameter implements Cloneable {
  ...
  public void addValues(float[] values) {
    for (int i = 0; i < values.length; i++) {
      this._values.addElement(new Float(values[i]));
    }
  }

  @Override
  public Object clone() {
    final DFloatList fl = (DFloatList) super.clone();
    fl._values = new Vector();
    for (int i = 0; i < _values.size(); i++) {
      fl.addValue(((Float) _values.elementAt(i)).floatValue());
    }
    return fl;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof DFloatList)) {
      return false;
    }

    return equals((DFloatList) obj);
  }
