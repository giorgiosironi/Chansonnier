...
public abstract class DIntegerListCodec {
  ...
  public static DParameter decode(Element element) throws ParameterException {
    ...
    final DIntegerList dParameter = new DIntegerList();
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if ("Value".equals(node.getLocalName())) {
        dParameter.addValue(Integer.parseInt(text));
      }
    }
  ...
  public static Element encode(DIntegerList dParameter, Element element) throws ParameterException {

    final Document doc = element.getOwnerDocument();
    final Element el = doc.createElementNS(DParameterSetCodec.NS, "Parameter");
    el.setAttribute("xsi:type", "IntegerList");

    final int[] values = dParameter.getValues();
